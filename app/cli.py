...
# ======================================================================
# Re: creating a function to instantiate the whole app:
# Another module that was tricky was app/cli.py, which implements a few
# shortcut commands for managing language translations. The current_app
# variable does not work in this case because these commands are registered
# at start up, not during the handling of a request, which is the only time
# when current_app can be used. To remove the reference to app in this module,
# I resorted to another trick, which is to move these custom commands inside
# a register() function that takes the app instance as an argument:
# ======================================================================
def register(app):
	@app.cli.group()
	def translate():
